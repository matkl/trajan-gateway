...
function Socket(address) {
  ...
  this.connection = net.createConnection({ host: pieces[0], port: pieces[1] });
...
Socket.prototype.onConnect = function() {
  debug('connected to ' + this.connection.remoteAddress + ':' + this.connection.remotePort);
  this.emit('connect');
};
...
Socket.prototype.onClose = function(hadError) {
  setTimeout(this.connection.connect.bind(this.connection), 5000);
  debug('close');
